...
pub(crate) fn build_unimod_xml_ontology(out_dir: &Path) {
...
fn parse_mod(node: &Node) -> Result<OntologyModification, String> {
    ...
    for child in node.children() {
        ...
        if child.has_tag_name("xref") {
            let source = child // For Misc. URL handling could be better
                .children()
